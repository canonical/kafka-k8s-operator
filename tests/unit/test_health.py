...
def test_service_pid(harness: Harness[KafkaCharm]):
    with (
        patch(
            "builtins.open",
        ...
        assert harness.charm.broker.health._service_pid == 1314231


def test_check_vm_swappiness(harness: Harness[KafkaCharm]):
    with (
        ...
        assert not harness.charm.broker.health._check_vm_swappiness()
        assert not harness.charm.broker.health.machine_configured()
...
def test_check_total_memory_testing_profile(
    harness: Harness[KafkaCharm], total_mem_kb, profile, limit
):
    ...
    with patch("workload.KafkaWorkload.read", return_value=[f"MemTotal:      {total_mem_kb} kB"]):
        if total_mem_kb / 1000000 <= limit:
            assert not harness.charm.broker.health._check_total_memory()
        else:
            assert harness.charm.broker.health._check_total_memory()


def test_get_partitions_size(harness: Harness[KafkaCharm]):
    ...
    with patch("workload.KafkaWorkload.run_bin_command", return_value=example_log_dirs):
        assert harness.charm.broker.health._get_partitions_size() == (5, 393)


def test_check_file_descriptors_no_listeners(harness: Harness[KafkaCharm]):
...
def test_machine_configured_succeeds_and_fails(harness: Harness[KafkaCharm], mmap, fd, swap, mem):
    with (
        ...
        if all([mmap, fd, swap, mem]):
            assert harness.charm.broker.health.machine_configured()
        else:
            assert not harness.charm.broker.health.machine_configured()
